...
export const AgencySearchResults = (props) => {
	...
	const { isAgencyOverlay, setAgencyValue, setAgencyValueType} = props
	...
	const getAgencyValue = (agencyVal, agencyType) => {
		setAgencyValue(agencyVal)
		setAgencyValueType(agencyType)
	}
					...
					<AnimatePresence initial={{scale: 0, opacity: 0}}>
							<motion.li
								...
								className={i.type} key={i.id}>
									<span data-agency={i.name} onClick={()=>getAgencyValue(i.friendlyname,i.type)}>
										{(() => {
											switch(i.type) {
												case 'police':
